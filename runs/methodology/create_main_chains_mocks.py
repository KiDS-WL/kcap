...
def sample_random_from_multinest_EE_nE_w(data, wgt):
...
def sample_random_from_multinest_EE_nE(data, wgt):
    kernel = stats.gaussian_kde(data, 'silverman', weights=wgt)
    random_start = kernel.resample(1).flatten()
...
if __name__ == "__main__":
    ...
    # Multinest random for main chains FMS
    if args.multi_start[0] < args.multi_start[1]:
        run_type = 'EE_nE_w'
        multinest_prior_name = f'runs/methodology/data/multinest_prior/multinest_main_{run_type}_samples.npy'
        weight_name = f'runs/methodology/data/multinest_prior/multinest_main_{run_type}_weight.npy'
        data = np.load(multinest_prior_name)
        wgt  = np.load(weight_name)
        output_dir = f"runs/methodology/data/multinest_start/main_chains/{run_type}/"
        os.makedirs(f'{output_dir}', exist_ok=True)

        for i in range(args.multi_start[0], args.multi_start[1]):
            starting_point_settings = sample_random_from_multinest_EE_nE_w(data, wgt)
