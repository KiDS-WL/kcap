...
def setup(options):
    ...
    return mass, nmass, z_vec, nz, p_GG, p_nn, p_xgG, p_gI, p_xGI, p_II, gravitational, galaxy, alignment, \
           ia_lum_dep_centrals, ia_lum_dep_satellites, two_halo_only, f_red_cen, pipeline, hod_section_name, suffix
...
def execute(block, config):
    ...
    ia_lum_dep_centrals, ia_lum_dep_satellites, two_halo_only, f_red_cen, pipeline, hod_section_name, suffix = config
    ...
    if two_halo_only == True:
        ...
        if p_xgG == True:
            ...
            pk_1h, pk_2h, pk_tot = compute_p_xgG(block, k_vec, pk_eff, z_vec, mass, dn_dlnm, c_factor, s_factor, m_factor, I_c_term, I_s_term,
                          I_m_term)
        ...
        if p_gI == True:
            ...
            pk_gI_1h, pk_gI_2h, pk_gI = compute_p_gI(block, k_vec, pk_eff, z_vec, mass, dn_dlnm, c_factor, s_align_factor, I_c_term, I_s_term,
                         alignment_amplitude_2h, nz, nk)
            block.put_grid("galaxy_intrinsic_power" + suffix, "z", z_vec, "k_h", k_vec, "p_k", pk_gI)
        if p_xGI == True:
