...
class TwoPointFile(object):
    ...
    def plots(self, root, colormap='viridis', savepdf=False, latex=True, plot_spectrum=True, plot_kernel=True, plot_cov=True, cov_vmin=None, sharey=True, save_pickle=False, load_pickle=False, remove_pickle=True,label_legend ='', blind_yaxis=False, shade_until=None, callback=None):
        ...
        def corr_names(spectrum):
            ...
            elif (spectrum.type1 == spectrum.type2 == Types.galaxy_shear_emode_fourier):
                corr_type = 'xip'
                label = r"$P_{ee}(\ell)$"
                color = plt.get_cmap(colormap)(0)
                
            elif (spectrum.type1 == spectrum.type2 == Types.galaxy_shear_bmode_fourier):
                corr_type = 'xim'
                label = r"$P_{ee}(\ell)$"
                color = plt.get_cmap(colormap)(0.2)

            elif (spectrum.type1 == Types.galaxy_position_fourier) & (spectrum.type2 == Types.galaxy_shear_emode_fourier):
                corr_type = 'gt'
                label = r"$P_{ne}(\ell)$"
                color = plt.get_cmap(colormap)(0.4)

            elif (spectrum.type1 == spectrum.type2 == Types.galaxy_position_fourier):
                corr_type = 'wtheta'
                label = r"$P_{nn}(\ell)$"
                color = plt.get_cmap(colormap)(0.6)
            ...
            for spectrum in self.spectra:
                ...
                for k,pair in enumerate(pairs):
                    ...
                    if (not all(bins1 == bins2)) & (j == nbins2):
                        ax[j-1][i-1].set_xlabel(r"$\ell$")#r"$\theta$ [arcmin]")
                    if all(bins1 == bins2):
                        ax[j-1][i-1].set_xlabel(r"$\ell$")#r"$\theta$ [arcmin]")
                    if i == 1:
