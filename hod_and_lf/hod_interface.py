...
def setup(options):
	...
	magnitudes_z = options[option_section, "magnitudes_z"]
	abs_mag_sun = options[option_section, "abs_mag_sun"]

	if magnitudes_z:
		file_name = options[option_section, "magnitudes_file"]
		z_bins, min_abs_mag, max_abs_mag = load_data(file_name)
		nz = len(z_bins)
		log_lum_min = -0.4*(max_abs_mag-abs_mag_sun)
		log_lum_max = -0.4*(min_abs_mag-abs_mag_sun)
	else:
		min_abs_mag = options[option_section, "min_abs_mag"]
		max_abs_mag = options[option_section, "max_abs_mag"]
		nz = options[option_section, "nz"]
		log_lum_min = np.repeat(-0.4*(max_abs_mag-abs_mag_sun),nz)
		log_lum_max = np.repeat(-0.4*(min_abs_mag-abs_mag_sun),nz)
	...
	rescale_to_h = options[option_section, "rescale_to_h"]
	...
	number_density_option, galaxy_bias_option, lf_option, clf_quantities, rescale_to_h, suffix
...
def execute(block, config):
	...
	number_density_option, galaxy_bias_option, lf_option, clf_quantities, rescale_to_h, suffix = config 

	start_time = time.time()

	#Comology
	NewtonGravConst = 4.2994e-9
	Omega_m_0 = block["cosmological_parameters", "omega_m"]
	rho_crit_0 = (3.*10.**4.)/(8.*np.pi*NewtonGravConst)
	rho_aver_com = rho_crit_0*Omega_m_0 #units of M_solar Mpc^-3 h^2
	...
	if lf_option:
		...
		nl_obs = 100
		L_obs = np.logspace(6.0,16.0,nl_obs)		
		Lf_func = np.empty([nz,nl_obs])
		...
		#Back to magnitudes	
		#Lf_in_mags = np.empty([nz,nl_obs])
		#for jz in range(0,nz):
		#	Lf_in_mags[jz] = 0.4*np.log(10.)*L_h*Lf_func_h
	
		#block.put_double_array_1d("luminosity_function" + suffix,'mr', mr_obs)
		#block.put_double_array_1d("luminosity_function" + suffix,'lf_mr',Lf_in_mags)

		#Characteristic luminosity of central galaxies
		#Lc_cen = lf.L_c(mass, hod)

		#block.put_double_array_1d("luminosity_function" + suffix,'mass',mass)
		#block.put_double_array_1d("luminosity_function" + suffix,'Lc',Lc_cen)
