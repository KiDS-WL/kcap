...
def two_halo_truncation_ia(k_vec):
    # TODO: very rough, explore this better
    k_trunc = 50.0
    return 0.5*(1.0+(erf(-(k_vec-k_trunc))))
...
def compute_two_halo_alignment(block, suffix, nz, nk, growth_factor, mean_density0, luminosity_dependence):
    '''
    The IA amplitude at large scales.
    ...
    :param luminosity_dependence: bool, if True the IA signal has the power law dependence as in Joachimi et al. 2011.
...
def compute_p_xGI(block, k_vec, p_eff, z_vec, mass, dn_dln_m, m_factor, s_align_factor, alignment_amplitude_2h, nz, nk,
    ...
    # 1-halo term
    pk_sm_1h = - compute_1h_term(m_factor, s_align_factor, mass, dn_dln_m[:,np.newaxis]) * one_halo_truncation(k_vec)[np.newaxis,:]
    # prepare the 1h term
...
def compute_p_gI(block, k_vec, p_eff, z_vec, mass, dn_dln_m, c_factor, s_align_factor, I_c_term, I_s_term, alignment_amplitude_2h,
                 nz, nk):
    #
    # p_tot = p_cs_gI_1h + p_cc_gI_2h + O(p_ss_gI_1h) + O(p_cs_gI_2h)
    #
    # 2-halo term:
    # TODO: which weighting here? This must be tested! Does it returns the usual bg*NLA? Are all the terms summed correctly?
    ...
    pk_tot_2h = b_g * (p_eff * alignment_amplitude_2h) * two_halo_truncation(k_vec)[np.newaxis,:]
    
    # This is something we can try, but must I'm not particularly convinced, as I think here we are assuming the linear bias to be 
    #caused by the central galaxies only:
    #pk_cc_2h = compute_2h_term(p_eff, I_c_term, align_2h)
    #pk_cc_2h = compute_2h_term(p_eff, I_c_term, alignment_amplitude_2h[:,np.newaxis]) * two_halo_truncation(k_vec)[np.newaxis,:]
    # 1-halo term
    pk_cs_1h = compute_1h_term(c_factor[:,np.newaxis], s_align_factor, mass, dn_dln_m[:,np.newaxis]) * one_halo_truncation(k_vec)[np.newaxis,:]
    '''
    # prepare the 1h term
    pk_cs_1h = np.empty([nz, nk])
    for jz in range(0, nz):
    ...
    return pk_cs_1h, pk_tot_2h, pk_tot
