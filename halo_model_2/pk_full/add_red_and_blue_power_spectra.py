...
# NOTE that at the moment the blue power spectrum is actually 'blue' only on
# small scales as at large scales it coincides with the red one
# This means that we can sum them without problem at large scales too (as 
# they are the same and f_red+f_blue=1, but this is not smart computationally
# as we are computing the large scales twice for no reason
# -> in the future it will make sense to have a BLUE and a RED, separated, 
# power spectra (where the red takes the central fraction over the red sample
# for the large scales instead of the fraction of red centrals over the total
# -> check that the fractions scales properly (i.e. f_cen_in_red*f_red = f_red_cen...)
