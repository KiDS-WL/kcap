...
# This module is the same as photoz_bias.py in cosmosis-standard-library 
# expect that it also calculates the true mean_deltaz shift and puts it in block
...
def setup(options):
    ...
    output_section_name = options.get_string(option_section,"output_section_name","delta_z_out")
    if sample == "":
    ...
    return {"mode": mode, "sample": pz, "bias_section": bias_section, "interpolation": interpolation, "per_bin": per_bin, "output_section_name": output_section_name, "output_deltaz": output_deltaz }
...
def execute(block, config):
    ...
    for i in range(1, nbin + 1):
        ...
        if config["output_deltaz"]:
            ...
            delta_z = mean_z_shifted-mean_z_input
            block[config["output_section_name"],bin_name] = delta_z
        #
