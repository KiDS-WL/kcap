...
extern "C" 
{
		...
		else
		{
			...
			if(type_str=="clustering")
			{
				...
				if (status) 
				{
					...
					clog<<" setting to default: "<< GALAXY_XI_SECTION <<endl;
					config->input_section_name=GALAXY_XI_SECTION;
				}
				else
			...
			else if(type_str=="ggl")
			{
				...
				{
					clog<<"Could not load input_section_name to band power,";
					clog<<" setting to default: "<< "galaxy_shear_xi" <<endl;
					config->input_section_name="galaxy_shear_xi";
				}
				else
					clog<<"got the value of input_section_name:"<<config->input_section_name<<endl;
			}
			else if(type_str=="cosmic_shear_e")
			{
				...
				{
					clog<<"Could not load input_section_name to band power,";
					clog<<" setting to default: "<< SHEAR_XI_SECTION <<endl;
					config->input_section_name=string(SHEAR_XI_SECTION)+"_plus";
				}
				else
					clog<<"got the value of input_section_name:"<<config->input_section_name<<endl;

				status=options->get_val<string>(sectionName, string("input_section_name_minus"), config->input_section_name_minus);
				if (status) 
				{
					clog<<"Could not load input_section_name to band power,";
					clog<<" setting to default: "<< SHEAR_XI_SECTION <<endl;
					config->input_section_name=string(SHEAR_XI_SECTION)+"_minus";
				}
				else
					clog<<"got the value of input_section_name:"<<config->input_section_name<<endl;
	...
	DATABLOCK_STATUS execute(cosmosis::DataBlock *block, void *config_in) 
	{
		...
		status = block->get_val(config->input_section_name, string("theta"), ell);
		if (status) 
		{
			clog<<"Could not load theta in "<<config->input_section_name<<" to BandPower"<<endl;
			return status;
		}
		...
		vector <vector<number> > Input_vec_vec;
		int nPairs=0;
		for (int i_bin=1; i_bin<=num_z_bin_A; i_bin++) 
		{
			for (int j_bin=i_bin; j_bin<=num_z_bin_B; j_bin++) 
			{
				...
				if (status) 
				{
					clog<<"Could not load bin "<<j_bin<<"_"<< i_bin<<" in xi to bandpower"<<endl;
					return status;
				}
				Input_vec_vec.push_back(C_ell);
				///put Cl in a vector to send to COSEBIs.
				nPairs++;
			}
		}

		vector <vector<number> > Input_minus_vec_vec;
		if(config->type>2)
		{
			int nPairs=0;
			for (int i_bin=1; i_bin<=num_z_bin_A; i_bin++) 
			{
				for (int j_bin=i_bin; j_bin<=num_z_bin_B; j_bin++) 
				{
					...
					vector<number> C_ell;
					string name_in=string("bin_")+toString(j_bin)+string("_")+toString(i_bin);
					status = block->get_val(config->input_section_name_minus, name_in, C_ell);
					//string name_Cl=string("bin_")+toString(j_bin)+string("_")+toString(i_bin)+string(".ascii");
					//clog<<"name_in="<<name_in<<endl;
					//status = block->put_val<vector<number> >(config->output_section_name, name_Cl, C_ell);
					if (status) 
					{
						clog<<"Could not load bin "<<j_bin<<"_"<< i_bin<<" in xi minus to bandpower"<<endl;
						return status;
					}
					Input_vec_vec.push_back(C_ell);
					///put Cl in a vector to send to COSEBIs.
		...
		if(config->type==0)
		{
			...
			config->BP0->setZbins(nPairs);
			config->BP0->setInput(logell,Input_vec_vec);
		...
		else if(config->type==2)
		{
			...
			config->BP2->setInput(logell,Input_vec_vec);
		...
		else if(config->type==3)
		{
			...
			config->BP4->setInput(logell,Input_minus_vec_vec);
			matrix BP_mat4;
